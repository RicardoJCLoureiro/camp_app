...
export default function UserDetailsPage() {
  ...
  const changedPayload = useMemo(() => {
    ...
    const out: any = {};
    (Object.keys(form) as (keyof UserDetails)[])
      .forEach(k => {
        if (k === 'userId' || k === 'email' || k === 'isActive' || k === 'isMfaEnabled') return;
  ...
  const uploadAvatar = async () => {
    ...
    try {
      ...
      // set into form (will be included when saving)
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      // If language changed, we’ll prompt after save
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl bg-white p-4 shadow ring-1 ring-black/5 space-y-3">
          ...
          <div>
            ...
            <input className="mt-1 w-full border rounded p-2"
              value={form.surname} onChange={e => onChange('surname', e.target.value)} />
          </div>
          <div>
            <label className="text-sm text-gray-600">{t('users.fields.languagePreference')}</label>
            <input className="mt-1 w-full border rounded p-2"
              value={form.languagePreference || ''} onChange={e => onChange('languagePreference', e.target.value)} />
          ...
          <div>
            ...
            <input className="mt-1 w-full border rounded p-2"
              value={form.profilePictureUrl || ''} onChange={e => onChange('profilePictureUrl', e.target.value)} />
          </div>
        </div>
        ...
        <div className="rounded-2xl bg-white p-4 shadow ring-1 ring-black/5 space-y-3">
          <div>
            ...
            <input className="mt-1 w-full border rounded p-2"
              value={form.phoneNumber || ''} onChange={e => onChange('phoneNumber', e.target.value)} />
          </div>
          <div>
          ...
          <div>
            ...
            <input className="mt-1 w-full border rounded p-2"
              value={form.addressLine2 || ''} onChange={e => onChange('addressLine2', e.target.value)} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input className="mt-1 w-full border rounded p-2"
                value={form.stateProvince || ''} onChange={e => onChange('stateProvince', e.target.value)} />
            </div>
            <div>
      ...
      <div className="flex gap-2">
        ...
        <Button variant="secondary" onClick={() => setForm(original!)}>{t('common.reset')}</Button>
      </div>
    </div>
  );
}
